...
{
    private static AIDIPTelemetryQueue Queue(int capacity) =>
        new(Options.Create(new AIDIPOptions { QueueCapacity = capacity }));
...
{
    private static (AIDIPMiddleware Middleware, AIDIPTelemetryQueue Queue, AIDIPMetrics Metrics) Build(
        RequestDelegate next,
        Action<AIDIPOptions>? configure = null)
    {
        var options = new AIDIPOptions { ProjectId = Guid.NewGuid(), ApplicationName = "TestApp" };
        configure?.Invoke(options);

        var wrapped = Options.Create(options);
        var queue = new AIDIPTelemetryQueue(wrapped);
        var metrics = new AIDIPMetrics();

        return (new AIDIPMiddleware(next, queue, metrics, wrapped), queue, metrics);
    }
    ...
    public async Task AnExceptionIsCapturedAndStillPropagates()
    {
        ...
        // handling has to keep working exactly as it did before AIDIP was added.
