...
public class FakeAiService : IAiMicroservice
{
    ...
    public Task<AIDIP.Backend.DTOs.ErrorAnalysisResponse> AnalyzeErrorAsync(string log, CancellationToken ct = default)
        => Task.FromResult(new AIDIP.Backend.DTOs.ErrorAnalysisResponse { RootCause = "fake" });

    public Task<AIDIP.Backend.DTOs.PredictionResponse> PredictAsync(List<string> recentLogs, string currentLog, CancellationToken ct = default)
        => Task.FromResult(new AIDIP.Backend.DTOs.PredictionResponse());

    public Task<AIDIP.Backend.DTOs.RecommendationResponse> RecommendAsync(string context, CancellationToken ct = default)
        => Task.FromResult(new AIDIP.Backend.DTOs.RecommendationResponse());

    public Task<List<AIDIP.Backend.DTOs.FixSuggestionDto>> SuggestFixesAsync(
        List<AIDIP.Backend.DTOs.MismatchDto> mismatches, CancellationToken ct = default)
        => Task.FromResult(new List<AIDIP.Backend.DTOs.FixSuggestionDto>());
}
