...
builder.Services.AddAIDIP(options =>
{
    ...
    options.ApplicationName = "AIDIP.DemoApp";
    options.ServiceName = "OrderProcessingService";
    options.Environment = "Demo";
    options.MetricsIntervalSeconds = builder.Configuration.GetValue("AIDIP:MetricsIntervalSeconds", 5);
});
...
// --- Control API ---
//
// The closed set of commands the AIDIP backend's registered remediation tools may issue. There is
...
app.MapGet("/aidip-control/state", (DemoScenario scenario) => Results.Ok(scenario.GetState()));

app.MapPost("/aidip-control/{command}", (string command, DemoScenario scenario) =>
{
