...
public sealed class SqliteBackupService : ISqliteBackupService
{
    ...
    public Task<BackupResult> CreateAsync(string reason, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            paths.EnsureCreated();
            var sourcePath = string.IsNullOrWhiteSpace(_options.DatabasePath) ? paths.DatabasePath
...
public sealed class PersistenceMaintenanceService : BackgroundService
{
    ...
    { _scopes = scopes; _options = options.Value; _state = state; _logger = logger; }
    ...
    {
        _state.LastStartedAt = DateTime.UtcNow; _state.Status = "Running"; _state.LastError = null;
        try
        {
            ...
            var deleted = 0;
            deleted += await db.TelemetryReceipts.Where(x => x.ReceivedAt < now.AddDays(-Days(_options.RawTelemetryRetentionDays, 1)))
                .ExecuteDeleteAsync(cancellationToken);
            deleted += await db.Metrics.Where(x => x.Timestamp < now.AddDays(-Days(_options.LegacySignalRetentionDays, 1)))
                .ExecuteDeleteAsync(cancellationToken);
            deleted += await db.Incidents.Where(x => x.SreIncidentId == null
                    && x.Timestamp < now.AddDays(-Days(_options.LegacySignalRetentionDays, 1)))
                .ExecuteDeleteAsync(cancellationToken);
            ...
            var expiringIncidentIds = await db.SreIncidents.Where(x => terminal.Contains(x.Status)
                    && x.UpdatedAt < incidentCutoff).Select(x => x.Id).ToListAsync(cancellationToken);
            deleted += await db.Incidents.Where(x => x.SreIncidentId.HasValue
                    && expiringIncidentIds.Contains(x.SreIncidentId.Value) && x.Timestamp < incidentCutoff)
                .ExecuteDeleteAsync(cancellationToken);
            deleted += await db.SreIncidents.Where(x => terminal.Contains(x.Status)
                    && x.UpdatedAt < incidentCutoff)
                .ExecuteDeleteAsync(cancellationToken);
            deleted += await db.PlatformAuditEvents.Where(x => x.Timestamp < now.AddDays(-Days(_options.AuditRetentionDays, 30)))
                .ExecuteDeleteAsync(cancellationToken);
            _state.LastDeletedRows = deleted; _state.LastCompletedAt = DateTime.UtcNow; _state.Status = "Healthy";
            _logger.LogInformation("Persistence maintenance completed deleted={DeletedRows}", deleted);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { }
        catch (Exception exception)
        {
            _state.Status = "Degraded"; _state.LastError = "Persistence maintenance failed.";
