...
public interface IProjectCredentialService
{
    ...
    Task<bool> AuthorizeAsync(IReadOnlyCollection<Guid> projectIds, string? suppliedKey, CancellationToken cancellationToken);
}
...
public sealed class ProjectCredentialService : IProjectCredentialService
{
    ...
    public async Task<CreatedProjectCredential?> CreateAsync(Guid projectId, string name, CancellationToken cancellationToken)
    {
        ...
        var credential = new ProjectApiCredential { Id = Guid.NewGuid(), ProjectId = projectId,
            Name = string.IsNullOrWhiteSpace(name) ? "Telemetry" : name.Trim(), KeyPrefix = key[..12],
            KeyHash = Hash(key), CreatedAt = now };
    ...
    public async Task<bool> AuthorizeAsync(IReadOnlyCollection<Guid> projectIds, string? suppliedKey,
        CancellationToken cancellationToken)
    {
        if (!_options.RequireTelemetryKey) return true;
        if (projectIds.Count != 1 || string.IsNullOrWhiteSpace(suppliedKey) || suppliedKey.Length < 16) return false;
        var prefix = suppliedKey[..Math.Min(12, suppliedKey.Length)];
        var candidates = await _db.ProjectApiCredentials.AsNoTracking().Where(x =>
            x.ProjectId == projectIds.Single() && x.KeyPrefix == prefix && x.RevokedAt == null)
            .Select(x => x.KeyHash).ToListAsync(cancellationToken);
