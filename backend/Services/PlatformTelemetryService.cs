...
public sealed class PlatformTelemetryService : IPlatformTelemetryService
{
        ...
        NormalizedTelemetryBatchDto batch, CancellationToken cancellationToken)
    {
        ...
        return new NormalizedTelemetryResultDto(accepted.Count, duplicateIds.Count,
            rejected, accepted);
    }

    private async Task<KaironProject> EnsureProjectAsync(NormalizedTelemetryEventDto item, DateTime now,
        CancellationToken cancellationToken)
    {
        var project = await _db.Projects.FindAsync([item.ProjectId], cancellationToken);
        if (project is not null) return project;
        project = new KaironProject { Id = item.ProjectId, Name = item.Application,
            Slug = $"project-{item.ProjectId:N}", CreatedAt = now };
        _db.Projects.Add(project);
        return project;
    }

    private async Task<MonitoredApplication> EnsureApplicationAsync(KaironProject project,
        NormalizedTelemetryEventDto item, DateTime now, CancellationToken cancellationToken)
    {
        ...
        application = new MonitoredApplication { Id = Guid.NewGuid(), ProjectId = project.Id,
            Name = item.Application, Service = service, Runtime = item.Runtime, CreatedAt = now };
        _db.MonitoredApplications.Add(application);
        return application;
    }

    private async Task EnsureEnvironmentAsync(KaironProject project, string environment, DateTime now,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<TelemetrySourceRegistration> EnsureSourceAsync(KaironProject project,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (source is null)
        {
            source = new TelemetrySourceRegistration { Id = Guid.NewGuid(), ProjectId = project.Id,
                ApplicationId = application.Id, SourceType = item.Source, InstallationId = installation,
                Version = item.SourceVersion, RegisteredAt = now };
    ...
    private static TelemetryReceipt ToReceipt(NormalizedTelemetryEventDto item, Guid sourceId, DateTime now)
    {
        ...
        return new TelemetryReceipt { Id = Guid.NewGuid(), EventId = item.EventId, ProjectId = item.ProjectId,
            SourceId = sourceId, EventTimestamp = TimestampOf(item), EventType = item.EventType,
            Severity = item.Severity, Source = item.Source, Application = item.Application,
            Service = ServiceOf(item), Environment = EnvironmentOf(item),
            PayloadJson = Bound(JsonSerializer.Serialize(sanitized), 16000) ?? "{}", ReceivedAt = now };
    }
    ...
    private void AddCompatibilitySignal(NormalizedTelemetryEventDto item)
    {
        ...
        if (kind is "metric" or "metrics" or "resource" or "resource_metric")
        {
            ...
            _db.Metrics.Add(new Metric { Id = Guid.NewGuid(), ProjectId = item.ProjectId,
                Timestamp = TimestampOf(item), CpuPercent = values.CpuPercent, MemoryPercent = values.MemoryPercent,
                ResponseTimeMs = values.ResponseTimeMs,
                RequestCount = values.RequestCount, ErrorCount = values.ErrorCount, RetryCount = values.RetryCount,
                QueueDepth = values.QueueDepth, Application = item.Application, Service = ServiceOf(item),
                Component = item.Host, Environment = EnvironmentOf(item) });
            return;
        }
        ...
        _db.Incidents.Add(new Incident { Id = Guid.NewGuid(), ProjectId = item.ProjectId,
            Timestamp = TimestampOf(item), Endpoint = Redaction.Scrub(http?.Endpoint ?? dependency?.Target ?? item.Application) ?? item.Application,
            ...
            StackTrace = Bound(Redaction.Scrub(item.StackTrace), 8000), RequestId = Bound(Redaction.Scrub(item.RequestId), 200),
            Severity = item.Severity, Environment = EnvironmentOf(item), Application = item.Application,
            Service = ServiceOf(item), MetadataJson = Bound(JsonSerializer.Serialize(Sanitize(item.Metadata)), 16000) });
    }
    ...
    private static object Sanitize(NormalizedTelemetryEventDto item) => new
    {
        ...
        HttpContext = item.HttpContext is null ? null : new { Endpoint = Redaction.Scrub(item.HttpContext.Endpoint),
            item.HttpContext.Method, item.HttpContext.StatusCode, item.HttpContext.DurationMs },
        DependencyContext = item.DependencyContext is null ? null : new { item.DependencyContext.Name,
            Target = Redaction.Scrub(item.DependencyContext.Target), item.DependencyContext.Success,
            item.DependencyContext.DurationMs },
